...
<body class="is-preload">
	...
	<div id="main">
		...
		<!-- Posts -->
		<form method="post" action="/chalKag/sellBoardUpdate.do"
			...
			<div class="row gtr-uniform">
				...
				<div class="col-12 col-12-xsmall">
					<input type="text" name="title" id="title"
						value="${boardData.title}" required />
				</div>
				<!-- 상품명 -->
				<div class="col-6 col-12-xsmall">
					<input type="text" name="productName" id="productName"
						value="${boardData.productName}" required />
				</div>
				<!-- 가격 -->
				<div class="col-6 col-12-xsmall">
					<input type="text" name="price" id="price"
						value="${boardData.price}" required />
				</div>
				<!-- 종류 -->
				<div class="col-12 col-12-xsmall">
					<select name="productCategory" id="productCategory">
						<option value="DSLR"
							${boardData.productCategory == 'DSLR' ? 'selected' : ''}>DSLR</option>
						<option value="미러리스"
							${boardData.productCategory == '미러리스' ? 'selected' : ''}>미러리스</option>
						<option value="컴팩트"
							${boardData.productCategory == '컴팩트' ? 'selected' : ''}>컴팩트</option>
					</select>
				</div>
				<!-- 제조사 -->
				<!-- Break -->
				<div class="col-12 col-12-xsmall">
					<select name="company" id="company">
						<option value="캐논" ${boardData.company == '캐논' ? 'selected' : ''}>캐논</option>
						<option value="소니" ${boardData.company == '소니' ? 'selected' : ''}>소니</option>
						<option value="니콘" ${boardData.company == '니콘' ? 'selected' : ''}>니콘</option>
					</select>
				</div>
				...
				<div class="col-12 col-12-xsmall">
					<select name="state" id="state">
						<option value="판매중" ${boardData.state == '판매중' ? 'selected' : ''}>판매중</option>
						<option value="판매완료" ${boardData.state == '판매완료' ? 'selected' : ''}>판매완료</option>
					</select>
				</div>
				...
				<div style="width: 100%; display: flex; justify-content: center; align-items: center;">
					<div class="actions" id="previewDiv" style="display: none;">
						<input type="hidden" name="image" value="${boardData.image}">
						<img id="preview" style="max-width: 100%;" src="bimg/${boardData.image}" />
					</div>
				</div>
				<!-- 내용 -->
				<div class="col-12">
