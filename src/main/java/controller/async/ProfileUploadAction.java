...
/**
 * Servlet implementation class ProfileUploadAction
 */
...
public class ProfileUploadAction extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // UploadAction 클래스 위치의 경로를 찾아서 uploadDir에 대입
        // 확인된 위치:
        ...
        // .metadata 앞까지 문자열 자르고 이미지가 저장되는 폴더인 memberProfileImages까지의 절대경로 부여
        ...
        // memberID을 가져와서 사용자가 저장한 프로필 이미지 명으로 사용하려고 selectOne 사용
        ...
        if (memberDTO != null) {
            ...
            // 파일 이름 중복 시 사용할 사용자 정의 파일 리네임 정책 객체를 생성하고 전달 103번 라인 참조
            // 현재 이름을 무조건 현재 로그인된 id로 변경하고 저장
            ...
            if (memberDTO.getProfile() != null) {
                // 유저가 "티모.JPG"를 저장하고 "티모.PNG"를 저장할 때 rename에 의해서
                // "1.JPG"저장 후 "1.PNG"를 저장하는 과정에서 확장자를 제외한 "1"만 비교해서
                // 같은 게 있다면 기존 파일 삭제 후 새로운 파일 폴더에 저장하는 과정

                // 기존 파일이 저장되어 있는 절대 경로 확인
                String existingFilePath = uploadDir + File.separator + memberDTO.getProfile();

                // 기존 파일이 저장되어 있는 절대 경로를 가지고 있는 existingFile 객체 생성
                File existingFile = new File(existingFilePath);

                // 새로운 파일이 저장된 절대 경로 확인
                String newFilePath = uploadDir + File.separator + newFileName;
                
                // 새로운 파일이 저장되어 있는 절대 경로를 가지고 있는 newFile 객체 생성
                File newFile = new File(newFilePath);

                String existingFileName = memberDTO.getProfile();
                
                // 기존 파일이 존재하고 기존 파일의 확장자를 포함한 이름이 새로운 파일의 확장자를 포함한 이름과 같으면
                if (existingFile.exists() && existingFileName.equals(newFileName)) {
                    try {
                        ...
                        e.printStackTrace(); // 덮어씌움 실패하면 에러 처리 해야함
                    }
                    
                    // 확장자를 포함한 기존 파일과 새로운 파일의 이름이 다르고 "default_image.jpg" 가 아니라면 파일 삭제
...
class CustomFileRenamePolicy extends DefaultFileRenamePolicy {
    ...
    public File rename(File file) {
        ...
        // 업로드한 파일 확장자를 extension에 대입
        String extension = (extractExtension(file.getName()));

        // newName에 newFileName(memberNum 값) + ".확장자" 대입
