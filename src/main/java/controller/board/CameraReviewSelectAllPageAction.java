...
public class CameraReviewSelectAllPageAction implements Action { // 카메라 리뷰 게시판 카테고리 페이지로 이동. 전미지
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		// selectAll 결과(BoardDTO 객쳬)를 담는 boardDatas라는 ArrayList 생성
		ArrayList<BoardDTO> boardDatas = new ArrayList<BoardDTO>();

		// 리뷰 게시판 카테고리의 게시글 전체 목록 받아오기
		boardDTO.setCategory("리뷰게시판");	
		boardDTO.setSearchCondition("");
		
//		System.out.println("[로그] CameraReviewSelectAllPageAction 1 카테고리 : " + boardDTO);
...
//		System.out.println("[로그] CameraReviewSelectAllPageAction 3" + boardDatas);
		...
		if (boardDatas != null) { // 게시글 정보(boardDatas)가 있다면
			...
			request.setAttribute("reviewBoardDTO", boardDatas);
			request.setAttribute("jsonReviewBoardDTO", jsonBoardDatas);
