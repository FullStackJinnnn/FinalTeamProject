...
public class SellBoardSelectAllPageAction implements Action { // 카메라 판매글 카테고리 페이지로 이동
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 판매(중고거래) 게시판 카테고리의 게시글 전체 목록 받아오기
		boardDTO.setCategory("판매게시판");	
		boardDTO.setSearchCondition("");
		
//		System.out.println("[로그] SellBoardSelectAllPageAction 1 카테고리 : " + boardDTO);

		// selectAll 메서드를 통해 데이터 베이스에서 모든 게시글 정보를 가져와서 boardDatas에 저장
		boardDatas = boardDAO.selectAll(boardDTO);
//		System.out.println("[로그] SellBoardSelectAllPageAction 2" + boardDTO)
//		System.out.println("[로그] SellBoardSelectAllPageAction 3" + boardDatas);
