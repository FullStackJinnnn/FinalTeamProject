...
FROM 
    (SELECT BOARD_DATA.*, ROWNUM, COALESCE(RECOMMEND_DATA.RECOMMENDCNT, 0) AS RECOMMENDCNT 
    FROM 
...
FROM (
    ...
    FROM (
        SELECT ROWNUM, BOARDNUM, ID, CATEGORY, TITLE, CONTENTS, TO_CHAR(BOARDDATE, 'YYYY-MM-DD') AS BOARDDATE,
    ...
    LEFT JOIN (
        ...
        FROM RECOMMEND
        GROUP BY BOARDNUM
    ) RECOMMEND_DATA ON BOARD_DATA.BOARDNUM = RECOMMEND_DATA.BOARDNUM ORDER BY BOARD_DATA.BOARDNUM DESC
) FINAL_DATA 
JOIN MEMBER ON MEMBER.ID = FINAL_DATA.ID;
...
FROM (
    SELECT 
        BOARD_DATA.*, 
        COALESCE(RECOMMEND_DATA.RECOMMENDCNT, 0) AS RECOMMENDCNT 
    FROM (
        SELECT 
            ROWNUM, BOARDNUM, ID, CATEGORY, TITLE, CONTENTS, TO_CHAR(BOARDDATE, 'YYYY-MM-DD') AS BOARDDATE, 
            PRICE, PRODUCTCATEGORY, COMPANY, STATE, VIEWCOUNT 
        FROM 
            BOARD 
        WHERE 
...
JOIN 
    MEMBER ON MEMBER.ID = FINAL_DATA.ID;
...
SELECT SORT_DATA.*, MEMBER.NICKNAME, MEMBER.ID FROM (SELECT ROWNUM, FILTER_DATA.*, COALESCE(RECOMMEND_COUNT.RECOMMENDCNT, 0) AS RECOMMENDCNT FROM (SELECT BOARDNUM, ID, CATEGORY, TITLE, CONTENTS, TO_CHAR(BOARDDATE, 'YYYY-MM-DD') AS BOARDDATE, PRICE, PRODUCTCATEGORY, COMPANY, STATE, VIEWCOUNT FROM BOARD WHERE CATEGORY = '리뷰게시판'    ORDER BY BOARDNUM DESC) FILTER_DATA LEFT JOIN (SELECT BOARDNUM, COUNT(BOARDNUM) AS RECOMMENDCNT FROM RECOMMEND GROUP BY BOARDNUM) RECOMMEND_COUNT ON FILTER_DATA.BOARDNUM = RECOMMEND_COUNT.BOARDNUM ) SORT_DATA JOIN MEMBER ON MEMBER.ID = SORT_DATA.ID  ORDER BY SORT_DATA.BOARDNUM DESC