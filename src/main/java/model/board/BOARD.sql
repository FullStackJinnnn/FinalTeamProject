...
---------REALISTIC DUMMY DATA----------
 ...
 INSERT INTO BOARD(BOARDNUM,ID,CATEGORY,TITLE,CONTENTS,PRICE,COMPANY,PRODUCTCATEGORY,VIEWCOUNT)
	VALUES((SELECT NVL(MAX(BOARDNUM),0)+1 FROM BOARD),'rornfl@naver.com','리뷰게시판','캐논 RF24-105mm F2.8 L IS USM Z 리뷰','사에서도 감히 기획 못했던 상상속의 그 렌즈.
일단 이 스펙으로 설계하려면 무게와 크기가 부담스러워지고
상승되는 가격에 따른 수요도 감안해야 하는 제품화가 결코 쉽지 않은 렌즈다.
24-105mm는 만능 표준 줌 렌즈를 대변하는 초점거리로서
기동성을 극대화한 엔트리급 컴팩트 렌즈를 병행 운영하는 것이 일반적이다. ',4590000,'캐논','DSLR',892);
 INSERT INTO BOARD(BOARDNUM,ID,CATEGORY,TITLE,CONTENTS,PRICE,COMPANY,PRODUCTCATEGORY,VIEWCOUNT)
	VALUES((SELECT NVL(MAX(BOARDNUM),0)+1 FROM BOARD),'rornfl1@naver.com','리뷰게시판','캐논의 보급형 DSLR? 프로용 DSLR? : EOS 10D','Canon EOS 10D(이하 10D)는 2003년 4월 한국에 정식 발매되었다.
그 이후로 캐논의 보급형 DSLR EOS 300D가 발매되었고, 니콘도 이에 맞서 D70을 내놓으며 치열한 보급형 DSLR 전쟁이 벌어졌다.
하지만 니콘 D70의 맞상대는 사실상 10D가 맞아 보인다.
플라스틱 바디를 쓴 300D에 비해 10D는 마그네슘 바디로 마감을 했고, AF성능도 지금 살펴봐도 전혀 손색이 없다.',176100,'캐논','DSLR','판매완료');
 INSERT INTO BOARD(BOARDNUM,ID,CATEGORY,TITLE,CONTENTS,PRICE,COMPANY,PRODUCTCATEGORY,VIEWCOUNT)
	VALUES((SELECT NVL(MAX(BOARDNUM),0)+1 FROM BOARD),'rornfl3@naver.com','리뷰게시판','[첫인상] 니콘 | Nikon Z fc','카메라가좋아요123',1380000,'니콘','미러리스',701);
 INSERT INTO BOARD(BOARDNUM,ID,CATEGORY,TITLE,CONTENTS,PRICE,COMPANY,PRODUCTCATEGORY,VIEWCOUNT)
	VALUES((SELECT NVL(MAX(BOARDNUM),0)+1 FROM BOARD),'rornfl4@naver.com','리뷰게시판','소니|SonyE16mmF2.8Review','카메라가좋아요123',247600,'소니','컴팩트',344);
x
