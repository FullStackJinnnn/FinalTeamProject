...
<body class="is-preload">
	...
	<div id="main">
		...
		<section class="post">
			...
			<div>
				<!-- 상품 가격 -->
				<pre> Price : ${board.price} </pre>
				<!-- 상품 종류 -->
				<pre> ProductCategory : ${board.productcategory} </pre>
				<!-- 상품 이름 -->
				<pre> ProductName : ${board.productName} </pre>
				<!-- 상품 제조사 -->
				<pre> Company : ${board.company} </pre>
			</div>
			...
			<div>
				<pre> Content : </pre>
				<!-- 첨부 이미지 -->
				<img src="bimg/${board.image}">이미지 출력 위치 <br>
			...
			<div class="col-6 col-12-small" style="margin-top: 45px; text-align: right;">
				...
				<c:if test="${sessionScope.memberid!=null}">
					<button type="button" onclick="location.href='/chalKag/cameraReviewUpdatePage.do'">Update</button>
					<button type="button" onclick="location.href='/chalKag/cameraReviewDeletePage.do'">Delete</button>
				</c:if>			
				<!-- '글 작성', '현재 카테고리 목록으로' 버튼 생성 -->
				<button type="button" onclick="location.href='/chalKag/cameraReviewWritePage.do'">Write</button>
				<button type="button" onclick="location.href='/chalKag/cameraReviewSelectAllPage.do'">Tolist</button>
			</div>
