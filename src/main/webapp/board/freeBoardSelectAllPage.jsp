...
<%--  <% System.out.println("[로그] 데이터 확인 : "+request.getParameter("reviewBoardDTO")); %> --%>
	...
	<div id="main">
		...
		<!-- 자유 게시판 데이터 선택을 위한 폼 -->
		<form id="freeBoardSelectAll" method="POST" action="/chalKag/freeBoardSelectAllPage.do">
			<!-- featured 포스트 섹션 -->
		...
		<div class="table-wrapper" style="margin-top: 20px; ">
			<table class="alt" style="margin-top: 30px;">
				...
				<tbody>
					...
					<c:if test="${fn:length(freeBoardDTO) <= 0}">
						<tr>
							<td colspan="6" align="center">등록된 글이 없습니다! 가장 먼저 새 글을 작성해 주세요!</td>
						</tr>
					</c:if>
					<!-- 출력할 게시글 정보(boardData)가 있을 경우 반복문을 사용하여 전체 목록 출력 -->
						<c:if test="${fn:length(freeBoardDTO) > 0}">
							<c:forEach var="boardData" items="${freeBoardDTO}">
									<tr>
